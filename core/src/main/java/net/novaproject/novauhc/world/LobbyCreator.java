...
public class LobbyCreator {
    ...
    public static void cloneWorld(String cloneOfName, String newWorldName) {
        ...
        Bukkit.getLogger().info("Cloning world '" + cloneOf.getName() + "' into new world '" + newWorldName + "'");
        ...
        Bukkit.getScheduler().runTaskAsynchronously(Main.get(), () -> {
            ...
            Bukkit.getLogger().info("Phase 1 successfully finished in " + (System.currentTimeMillis() - start) + " ms (Asynchronously)");

            Bukkit.getScheduler().runTask(Main.get(), () -> {
                long start2 = System.currentTimeMillis();
                worldCreator.createWorld();
                long current = System.currentTimeMillis();
                Bukkit.getLogger().info("Phase 2 successfully finished in " + (current - start2) + " ms (Synchronously)");
                Bukkit.getLogger().info("Successfully cloned world '" + cloneOf.getName() + "' into new world '" + newWorldName + "' in " + (current - start) + " ms.");
    ...
    public static void createWorld(String worldName, World.Environment environment) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(Main.get(), () -> {
            ...
            Bukkit.getScheduler().runTask(Main.get(), () -> {
                ...
                long current = System.currentTimeMillis();
                Bukkit.getLogger().info("Phase 2 successfully finished in " + (current - start2) + " ms (Synchronously)");
                Bukkit.getLogger().info("Successfully created new world '" + worldName + "' in " + (current - start) + " ms.");
    ...
    public static void deleteWorld(String worldName, Location fallBackLocation) {
        ...
        Bukkit.getLogger().info("Phase 1 successfully finished in " + (System.currentTimeMillis() - start) + " ms (Synchronously)");

        Bukkit.getScheduler().runTaskAsynchronously(Main.get(), () -> {
            long start2 = System.currentTimeMillis();
            deleteDirectory(worldFolder);
            long current = System.currentTimeMillis();
            Bukkit.getLogger().info("Phase 2 successfully finished in " + (current - start2) + " ms (Asynchronously)");
            Bukkit.getLogger().info("Successfully deleted world '" + worldName + "' in " + (current - start) + " ms.");
            worldsBeingTasked.remove(worldName.toLowerCase());
        });
    }
